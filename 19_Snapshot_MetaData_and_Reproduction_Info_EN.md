...
1. **Restoration-sufficient for continuing the simulation**: pose, velocity, joint state, status, connections. This is what the bulk of the existing snapshot schema is intended to provide, including the joint-state extension in [18_Snapshot_Alignment_with_Learning_Data_EN.md](./18_Snapshot_Alignment_with_Learning_Data_EN.md) — required regardless of whether images are ever regenerated, once that extension's still-open exact shape (Open Question 6) is settled; today it remains a non-normative illustration, not a finalized contract.
