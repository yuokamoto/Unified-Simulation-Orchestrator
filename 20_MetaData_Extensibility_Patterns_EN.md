...
The same underlying pattern — **a type/kind tag identifies which schema a payload follows; unrecognized types are passed through untouched; the schema for a given type is owned and versioned by whoever defined that type** — recurs across widely used systems:
...
- **CloudEvents** (CNCF) — an event carries a `type` and a `data` payload; the schema for `data` is owned by whoever defined that `type`; consumers that don't recognize the `type` can still route/store the event.
- **OCI** (container/artifact spec) — `mediaType` / `artifactType` fields declare how to interpret an otherwise opaque payload.
- **OpenAPI 3 / JSON Schema** — `discriminator` + `oneOf` formalize "one field's value selects which schema applies" as a first-class construct.
