...
- **Kubernetes** — `labels`/`annotations` (namespaced `<domain>/<name>` keys, subject to Kubernetes' own built-in syntax/length constraints but no user-defined schema validation) vs. **CustomResourceDefinitions** (a `kind` gets its own registered OpenAPI v3 schema, with `required` fields and validation, optionally preserving unknown fields).
