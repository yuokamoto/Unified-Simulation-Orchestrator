...
## シミュレータ選定の潮流（2025–2026）と、USOがフレームワークを固定しすぎない理由

> 後日（2026-08-31）の設計議論より記録。[11_Design_Principles_JP.md](./11_Design_Principles_JP.md) も参照。

物理／学習用シミュレータのエコシステムは動きが速く、これ自体が「シミュレータ層を固定せず差し替え可能に保つ」ことの根拠になっている。

- **レンダリング不要・状態ベースの方策学習**（MuJoCo系のMJX/Playgroundなど）は非常に高速なステップレートをマルチプラットフォームで達成でき、現行の多くのRL/VLA評価ハーネスもこのスタイルを前提にしている。
- **大規模並列の移動・全身制御**（GPU並列）はIsaac Labの強み。
- **フォトリアルな知覚学習・合成データ生成**（レンダリング必須）は引き続きIsaac Sim / Replicatorの強み。
- **統合が進行中**：Newton（NVIDIA・DeepMind・Disneyの協業で、MuJoCo-Warpバックエンド上に物理精度とGPU並列性の両立を狙う）や、mjlab（Isaac LabのAPIとMuJoCoの軽量性を組み合わせる）は、これらのツール間の境界が今後1〜2年で溶けていく可能性を示している。
- **USOへの含意**：アセットはUSD系に寄せておき、MuJoCo⇔Newton的な移行が起きても無駄にならないようにする一方、フレームワーク層をこれらのどれか一つに固定しすぎないこと。USOのマルチエンジン切り替え能力そのものが、この流動性に対する保険になっている。これは、[11_Design_Principles_JP.md](./11_Design_Principles_JP.md) の原則1が「どの学習フレームワークが今流行っているか」を、アセットやタスク論理から切り離すべき「変わる理由」として扱っていることの具体例である。
...
1. **gRPC（外部通信）**
   ...
   - GUI、CLI、サードパーティツールとSimulation Masterの接続におけるコアプロトコルとして機能。

2. **ROS 2（ラッパーとして）**
   - ロボティクス分野での標準的な通信基盤。
   - 実ロボットとの統合や、既存のROS2ノード群との接続が容易。
   - gRPCのラッパーとして使用することで、**シミュレーション内部の複雑さを回避しつつ、外部連携の利便性を確保**。

3. **ZeroMQ（内部状態同期）**
   - 内部通信（Master ⇔ Node）の唯一の通信手段。
