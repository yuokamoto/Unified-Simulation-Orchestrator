...
4. **Interface Layer（インターフェース層）**  
   ...
   - **内部通信（Master ⇔ Node）** には **ZeroMQ** を採用し、軽量・低遅延な差分スナップショット伝送を実現。  
   - **外部通信（GUI/制御系 ⇔ Master）** には **gRPC** をコア通信手段とし、必要に応じてROS2ラッパーを提供。
...
3. **Behavior Tree (BT)**  
   - **BT XML（BehaviorTree.CPP形式）** を正式な共通フォーマットとし、各ノードが自動変換して利用。  
   - SimPyではBT XMLを`py_trees`に変換、Gazeboや他エンジンでは`BehaviorTree.CPP`で直接実行。
