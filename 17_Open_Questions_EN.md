...
**Status:** Draft
**Date:** 2026-08-17 (items 1-5); updated 2026-08-31 (items 6-7 added); updated 2026-09-16 (item 7 resolved, items 8-12 added)
...
## 10. Should Snapshot Data Bindings Be Generated Per Language, With a Separately Shared Replay/Validation Implementation — and When?
...
## 12. Where Do Episode-Sampled, Dynamics-Affecting Domain-Randomization Values Live?

- Background: [03_Snapshot_Specification_EN.md](./03_Snapshot_Specification_EN.md) excludes dynamics-affecting domain randomization (friction, mass, actuator gains, and the like) from `reproduction_info`, on the grounds that such values change how the simulation continues and so must be treated as simulation state, not optional reproduction metadata. However, no normative field currently holds them: the only per-asset extension point is the freeform `properties` field, which is explicitly documented as carrying no guarantee that any consumer may rely on a given key being present. Where these episode-sampled values should be recorded so that a replay can deterministically reconstruct them (a new typed per-asset field, a convention for `properties`, or something else) is not yet decided.
- Related document: [03_Snapshot_Specification_EN.md](./03_Snapshot_Specification_EN.md), [19_Snapshot_MetaData_and_Reproduction_Info_EN.md](./19_Snapshot_MetaData_and_Reproduction_Info_EN.md)
